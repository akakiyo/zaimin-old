...
module GmailHelper
  def get_messages(token, query)
    ...
    mail_ids = get_mail_infos(token,uri)["messages"]
    if !mail_ids.nil?
  ...
  def get_payments(mail)
    ...
    for line in decoded_data.split(/\R/) do
      ...
      if flag then
        splited_payments.push(extract_payment_info(line.gsub(/>*/,"").split))
      end
  ...
  def extract_payment_info(line)
    ...
    {"date_of_use": date_of_use,"product_name": product_name,"price": price,"due_date": due_date}
  end
